  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        item.status === 'processing' ? "border-orange-500/30 shadow-lg shadow-orange-500/5 bg-neutral-900/70" :
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {item.status === 'processing' && (
            <span className="text-[10px] bg-orange-500/15 text-orange-300 border border-orange-500/20 px-2 py-0.5 rounded-full font-bold flex items-center gap-1 animate-pulse">
      ...
      >
        ...
        {item.status === 'completed' && (
          <div className="absolute top-2.5 left-2.5 z-10">
            <button
              ...
              className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-black/80 hover:bg-orange-500 text-white border border-white/15 transition-all shadow-lg flex items-center gap-1 backdrop-blur-md cursor-pointer"
              title="فتح معاينة مكبرة بملء الشاشة (Zoom)"
            >
              <ZoomIn className="w-3 h-3 text-orange-300 group-hover/imgbox:scale-110 transition-transform" />
              <span>تكبير 🔍</span>
            </button>
          </div>
        )}
          ...
          {item.status === 'processing' && (
            ...
            >
              <div className="relative mb-2">
                <div className="absolute -inset-3 bg-orange-500/20 rounded-full blur-lg animate-pulse"></div>
                <div className="w-10 h-10 rounded-full border-2 border-neutral-800 border-t-orange-500 animate-spin flex items-center justify-center">
        ...
        {item.status === 'pending' && (
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/card:opacity-100 transition-opacity flex items-center justify-center gap-2">
            <button
              onClick={onCrop}
              className="p-2 rounded-lg bg-neutral-900/95 hover:bg-orange-500 border border-white/10 text-white transition-all scale-95 group-hover/card:scale-100 cursor-pointer"
      ...
      {item.status === 'error' && (
        <div className="p-2.5 rounded-xl bg-red-950/20 border border-red-500/20 text-right space-y-2">
          ...
          {onManageApiKey && (
            <button
              ...
              className="px-2.5 py-1 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 cursor-pointer shadow-sm shadow-orange-500/10"
            >
              <span>إدخال أو تغيير مفتاح Gemini 🔑</span>
            </button>
          )}
        </div>
      )}
      ...
      {item.status === 'completed' && item.variants && item.variants.length > 1 && (
        <div className="flex flex-col gap-1.5 bg-black/40 p-2.5 rounded-xl border border-white/5" dir="rtl">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[9px] bg-orange-500/10 text-orange-300 px-1.5 py-0.5 rounded-md border border-orange-500/10 font-bold font-mono">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected 
                      ? "border-orange-500 scale-105 ring-2 ring-orange-500/35 shadow-md shadow-orange-500/15" 
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-orange-900/90 text-[8px] text-white text-center py-0.5 font-bold font-sans">
                    {vIdx + 1}
                  </div>
                </button>
              );
      ...
      <div className="mt-2 border-t border-white/5 pt-2.5 w-full">
          ...
          <div className="flex flex-col gap-1.5 w-full">
            ...
            <div className="grid grid-cols-3 gap-1.5 w-full">
              <button
                ...
                className="py-1.5 px-1.5 rounded-lg bg-orange-500 hover:bg-orange-600 text-white text-[10px] sm:text-[11px] font-bold transition-all flex items-center justify-center gap-1 shadow-md shadow-orange-500/10 cursor-pointer border border-orange-400/30 active:scale-95"
              ...
              <button
                ...
                className="py-1.5 px-1.5 rounded-lg bg-orange-950/50 hover:bg-orange-800/60 text-orange-200 hover:text-white text-[10px] sm:text-[11px] font-bold border border-orange-500/30 transition-all flex items-center justify-center gap-1 cursor-pointer active:scale-95"
                title="تعديل هذه النتيجة بالفرشاة أو مسح أجزاء منها"
              >
                <Sparkles className="w-3.5 h-3.5 shrink-0 text-orange-300" />
        ...
        {item.status === 'processing' && (
          <div className="w-full flex items-center justify-between gap-2 py-1.5 px-1 text-xs">
            <span className="text-orange-400/90 font-medium truncate">يتم المعالجة مع صور الدفعة...</span>
            {onStop && (
              <button
                type="button"
                onClick={onStop}
