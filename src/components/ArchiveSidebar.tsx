        ...
        <>
          ...
          >
            <header className="relative overflow-hidden border-b border-white/10 px-5 pb-4 pt-5">
              <div className="absolute inset-0 bg-gradient-to-bl from-orange-700/25 via-amber-700/10 to-transparent" />
              <div className="relative flex items-start justify-between gap-4">
                <div className="flex gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-orange-400/20 bg-orange-500/15 text-orange-200 shadow-lg shadow-orange-500/10">
            ...
            {tab === 'sessions' ? (
              <section className="flex min-h-0 flex-1 flex-col">
                <div className="border-b border-white/5 bg-white/[0.025] p-3">
                  <button type="button" onClick={onSaveSession} disabled={activeItems.length === 0} className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-xs font-black text-white shadow-lg shadow-orange-500/15 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-35">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto p-4">
                  {sessions.map((session, index) => {
                    ...
                    return (
                      <article key={session.id} className={cn('overflow-hidden rounded-2xl border bg-white/[0.035] transition', isCurrent ? 'border-orange-400/40 ring-1 ring-orange-500/10' : 'border-white/8 hover:border-white/15')}>
                        <div className="p-3.5">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {isCurrent && <span className="shrink-0 rounded-md bg-orange-500/15 px-1.5 py-0.5 text-[8px] font-bold text-orange-300">الحالية</span>}
                              </div>
                        ...
                        <div className="flex gap-2 border-t border-white/5 bg-black/20 p-2.5">
                          <button type="button" onClick={() => onRestoreSession(session)} className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-xl bg-white/8 text-[11px] font-bold text-white transition hover:bg-orange-500">
              ...
              <section className="flex min-h-0 flex-1 flex-col">
                <div className="flex items-center gap-2 border-b border-white/5 bg-white/[0.025] p-3">
                  <button type="button" onClick={onRestoreAllImages} disabled={archiveItems.length === 0} className="flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-xs font-black text-white shadow-lg shadow-orange-500/15 disabled:opacity-35">
                ...
                <div className="grid flex-1 auto-rows-max grid-cols-2 gap-3 overflow-y-auto p-4">
                  {archiveItems.map((item, index) => {
                    ...
                    return (
                      <article key={item.id} className={cn('group overflow-hidden rounded-2xl border bg-white/[0.035]', active ? 'border-orange-500/40' : 'border-white/8 hover:border-white/15')}>
                        <div className="relative aspect-square overflow-hidden bg-black/40">
                          ...
                          {active && <span className="absolute right-2 top-2 rounded-lg border border-orange-400/20 bg-orange-700/80 px-2 py-1 text-[8px] font-bold text-white backdrop-blur">في مساحة العمل</span>}
                        ...
                        <div className="p-2.5">
                          ...
                          <button type="button" onClick={() => onActivateImage(item)} className={cn('flex h-8 w-full items-center justify-center gap-1 rounded-lg text-[10px] font-bold transition', active ? 'border border-orange-500/20 bg-orange-500/10 text-orange-300' : 'bg-white/8 text-white hover:bg-orange-500')}>
                  ...
                  {archiveItems.length === 0 && (
                    <div className="col-span-2">
                      <EmptyState icon={ImageIcon} title="مكتبة الصور فارغة" detail="ستظهر هنا آخر الصور المستخدمة تلقائيًا." />
                    </div>
                  )}
                </div>
              </section>
...
function TabButton({ active, onClick, icon: Icon, label, count }: { active: boolean; onClick: () => void; icon: typeof Archive; label: string; count: number }) {
  return (
    <button type="button" onClick={onClick} className={cn('flex h-10 items-center justify-center gap-2 rounded-xl border text-[11px] font-bold transition', active ? 'border-orange-500/30 bg-orange-500/15 text-orange-200' : 'border-transparent text-neutral-400 hover:bg-white/5 hover:text-white')}>
